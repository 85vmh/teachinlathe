...
class LatheFixture(QWidget):
    ...
    def __init__(self, fixture, is_active=False, parent=None):
        ...
        self.teachZMinus.clicked.connect(self._teachZMinusClicked)
        self.teachZMinus.setEnabled(is_active)
        self._setActive(is_active)

    def mousePressEvent(self, event):
        # Emit the signal with the fixture object when the widget is clicked
        self.onFixtureSelected.emit(self.fixture)
    ...
    def _teachZMinusClicked(self):
        self.zMinusLimit.fillWithCurrentValue()
        current_value = float(self.zMinusLimit.text())
        self.fixture.z_minus_limit = current_value
        self.zMinusLimitUpdated.emit(current_value)
